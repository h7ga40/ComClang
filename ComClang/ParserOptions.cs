...
using LanguageVersion = CppSharp.LanguageVersion;
...
namespace CppSharp
{
	...
	public class ParserOptions : CppParserOptions
	{
			...
			IEnumerable<CppSharp.AST.Module> modules, string file = null)
		{
			...
			for (int i = 0; i < IncludeDirsCount; ++i)
			{
				var include = GetIncludeDirs(i);
				options.AddIncludeDirs(include);
			}

			for (int i = 0; i < SystemIncludeDirsCount; ++i)
			{
			...
			for (int i = 0; i < UndefinesCount; ++i)
			{
				var define = GetUndefines(i);
				options.AddUndefines(define);
			}

			for (int i = 0; i < LibraryDirsCount; ++i)
			{
...
#pragma warning restore 162
		...
		private void SetupArguments()
		{
			...
			{
			case CppSharp.LanguageVersion.C99:
			case CppSharp.LanguageVersion.C99_GNU:
				AddArguments("-xc");
				break;
			default:
				AddArguments("-xc++");
			...
			case CppSharp.LanguageVersion.C99_GNU:
				...
				break;
			case CppSharp.LanguageVersion.CPP98:
				AddArguments("-std=c++98");
				break;
			case CppSharp.LanguageVersion.CPP98_GNU:
				AddArguments("-std=gnu++98");
				break;
			case CppSharp.LanguageVersion.CPP11:
				AddArguments("-std=c++11");
				break;
			case CppSharp.LanguageVersion.CPP11_GNU:
				AddArguments("-std=gnu++11");
				break;
			case CppSharp.LanguageVersion.CPP14:
				AddArguments("-std=c++14");
				break;
			case CppSharp.LanguageVersion.CPP14_GNU:
				AddArguments("-std=gnu++14");
				break;
			case CppSharp.LanguageVersion.CPP17:
				AddArguments("-std=c++1z");
				break;
			case CppSharp.LanguageVersion.CPP17_GNU:
